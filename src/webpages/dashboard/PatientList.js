...
export class PatientList {
    ...
    showLoading() {
        ...
        this.container.innerHTML = `
            ...
            <div class="text-center">
                ...
                <p class="mt-2 text-sm text-muted-foreground">Loading patients...</p>
            </div>
            </div>
        `;
    }
    ...
    getFilteredPatients() {
        const patients = patientStore.patients;  // Changed from getPatients() to .patients
        return patients.filter(p => {
            ...
            const matchesDoctor =
                this.doctorFilter === "all" || p.assignedDoctor === this.doctorFilter;
    ...
    updateContent() {
        ...
        const uniqueDoctors = [
            ...new Set(patients.map(p => p.assignedDoctor).filter(Boolean))
        ];
            ...
            <div class="flex flex-col gap-3 p-4 rounded-xl bg-muted/30 border border-border">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <div class="relative flex-1">
                        <span id="search-icon" class="absolute left-3 top-1/2 -translate-y-1/2"></span>
                        <input
                            id="search-input"
                            placeholder="Search..."
                            value="${this.searchQuery}"
                            class="w-full h-10 pl-10 pr-4 rounded-lg border border-input bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                        />
                    </div>
        ...
        if (searchIcon) {
            searchIcon.appendChild(
                icons.search("h-4 w-4 text-muted-foreground")
            );
        }
        ...
        [
            ...
            { label: "Waiting", value: counts.waiting, filter: "waiting" },
            { label: "Checked In", value: counts.checkedIn, filter: "checked-in" },
            { label: "Completed", value: counts.completed, filter: "completed" }
        ].forEach(stat => {
            const btn = document.createElement("button");
            btn.className = `flex items-center gap-3 p-3 rounded-xl border transition-all ${
                this.statusFilter === stat.filter
